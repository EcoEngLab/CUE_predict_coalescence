...
params_df = pd.read_csv("coal_theory_curve_parameters.csv")
...
for grp in ["Community 1", "Community 2", "Neither"]:
...
ax.set_xlabel(r"$\Delta$Community-level CUE (Community 1 - Community 2)")
ax.set_ylabel(r"$\Delta$Similarity (Community 1 - Community 2)")
...
ax.set_xticklabels(
    [f"{iv.left:.2f} to {iv.right:.2f}" for iv in interval_order],
    rotation=40,
    ha="right"
)
style_ax(ax, grid=False)
...
for ax, dil in zip(axes, [0.01, 0.1]):
    ...
    survived = pivot["Survived"].values if "Survived" in pivot.columns else np.zeros(len(pivot))
    extinct = pivot["Extinct"].values if "Extinct" in pivot.columns else np.zeros(len(pivot))
